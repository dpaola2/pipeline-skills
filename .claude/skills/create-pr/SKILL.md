---
name: create-pr
description: "Push the implementation branch and create a PR against staging with a generated summary from project artifacts."
...
You **push the implementation branch and create a GitHub pull request** against `staging` with a summary generated from the project's pipeline artifacts. This is the delivery step after all pipeline stages are complete.
...
### Check 3: Branch Exists in Rails Repo

In the Rails repo (`~/projects/orangeqc/orangeqc/`), verify the project branch exists:

```bash
cd ~/projects/orangeqc/orangeqc && git branch --list 'pipeline/$ARGUMENTS'
```

If the branch does not exist, **STOP**:

> "The branch `pipeline/$ARGUMENTS` does not exist in the Rails repo."
...
> "The Rails repo has uncommitted changes. Please commit or stash them before creating a PR."
...
```bash
cd ~/projects/orangeqc/orangeqc && gh pr list --head 'pipeline/$ARGUMENTS' --state open --json number,url
```
...
Generated by the OrangeQC Agent Pipeline
```

### 4. Push the Branch

```bash
cd ~/projects/orangeqc/orangeqc && git push -u origin pipeline/$ARGUMENTS
```
...
```bash
cd ~/projects/orangeqc/orangeqc && gh pr create \
  --base staging \
  --head "pipeline/$ARGUMENTS" \
  --title "[title]" \
  --body "$(cat <<'EOF'
[generated PR body]
EOF
...
- **Do not push to `main` or `master`.** Always push to the `pipeline/$ARGUMENTS` branch.
