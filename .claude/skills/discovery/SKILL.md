...
- **Input:** `wcp_get_artifact($ARGUMENTS, "prd.md")`
- **Output:** `wcp_attach($ARGUMENTS, ...)` → `discovery-report.md`
...
2. The PRD: `wcp_get_artifact($ARGUMENTS, "prd.md")` — understand what we're building
...
Tell the user:
1. The discovery report has been attached to `$ARGUMENTS` as `discovery-report.md`
